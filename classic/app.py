...
def reload_player_on_hero():
    ...
    for hero in requests.get('https://api.opendota.com/api/heroes').json():
        hero_name_lookup[str(hero['id'])] = hero['localized_name'].lower().replace(
            '\'', '').replace(' ', '-')
    sleep(1)
    ...
    for id in [player.steamID for player in Player.query.all()]:
        ...
        hero_data = requests.get(
            'https://api.opendota.com/api/players/' + id + '/heroes').json()
...
def threaded_reload(response_url):
    ...
    data = {'response_type': 'in_channel',
            'text': 'Reloading temporarily disabled to avoid hitting rate limits.'}
    requests.post(url=response_url, json=data)
...
def recommendations():
    ...
    for hero in current_draft['rec_pool']:
        ...
        for enemy_hero in current_draft['enemies']:
            matchup = next(
                x for x in all_hero_v_hero_matchups if x.thishero == hero)
            hero_data[matchup.thishero][enemy_hero] = float(
                getattr(matchup, enemy_hero))/100
    ...
    for hero in current_draft['rec_pool']:
        ...
        for player in current_draft['players']:
            ...
            num_games = getattr(
                next(x for x in all_player_game_counts if x.playerID == playerID), hero)
            winrate_on_hero = getattr(
                next(x for x in all_player_winrates if x.playerID == playerID), hero)

            if int(num_games) > 4:
                winrate_against_enemies = float(
                    stats.gmean(list(hero_data[hero].values())))
                combined_winrate = float(stats.gmean(
                    [float(winrate_on_hero), winrate_against_enemies]))
                results.append((hero, player, str(winrate_on_hero), str(
                    winrate_against_enemies), str(combined_winrate)))

    results.sort(key=lambda x: x[4], reverse=True)
    for result in results[:5]:
        response += '*' + result[0] + '* - global ' + str(round(float(
            result[3])*100, 2)) + '%, ' + result[1] + ' ' + str(round(float(result[2])*100, 2)) + '%\n'
